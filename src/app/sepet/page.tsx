...
import { FREE_SHIPPING_THRESHOLD, formatPrice, products } from "@/lib/catalog";
...
export default function CartPage() {
  ...
  const lines = cart.map(line => ({ line, product: products.find(p => p.slug === line.slug) })).filter(x => x.product);
    ...
    <div className="cart-layout">
      <section className="cart-items">
        ...
        {lines.length === 0 ? <div className="empty-cart">
          <span>SEPETİNİZ BOŞ</span><h2>Henüz ürün eklemediniz.</h2><Link href="/kadin">Alışverişe başla →</Link>
        </div> : lines.map(({ line, product }) => <div className="cart-line" key={line.slug + line.size + line.color}>
          ...
          <div className="cart-line-main">
            ...
            <p>{line.size && "Beden: " + line.size}{line.color && " · Renk: " + line.color}</p>
            <div className="cart-qty"><button onClick={() => setQty(line.slug, line.qty - 1)}><Minus size={14}/></button><span>{line.qty}</span><button onClick={() => setQty(line.slug, line.qty + 1)}><Plus size={14}/></button></div>
          </div>
      ...
      <aside className="cart-summary">
        ...
        <div><span>Kargo</span><b>{quote.free ? "Ücretsiz" : hasUnknownPrice ? "Hesaplanacak" : formatPrice(quote.fee)}</b></div>
        <div className="summary-total"><span>Toplam</span><b>{hasUnknownPrice ? "—" : formatPrice(subtotal + quote.fee)}</b></div>
        {lines.length > 0 ? <Link href="/checkout">Alışverişi Tamamla</Link> : <span className="disabled-checkout">Alışverişi Tamamla</span>}
        <small>Ödeme sağlayıcısı henüz seçilmedi. Checkout ve sipariş akışı sağlayıcıdan bağımsız hazır.</small>
      </aside>
    </div>
  </main><StoreFooter/></>;
}
