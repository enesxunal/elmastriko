...
export default function CheckoutPage() {
  ...
  return <><StoreHeader/><main className="checkout-page">
    ...
    <aside className="checkout-summary"><h3>Sipariş Özeti</h3>{loading && cart.length > 0 ? <p>Sepet güncelleniyor...</p> : lines.length === 0 ? <p>Sepetiniz boş.</p> : lines.map(({ line, product }) => <div className="mini-product" key={line.slug + line.size + line.color}><img src={product.image} alt={product.name}/><p>{product.name}<br/><small>{line.qty} adet {line.size ? "· " + line.size : ""}{line.color ? " · " + line.color : ""}</small></p></div>)}<hr/><p><span>Ara toplam</span><b>{hasUnknownPrice ? "Fiyat listesi bekleniyor" : formatPrice(subtotal)}</b></p><p><span>Kargo</span><b>{shipping.free ? "Ücretsiz" : hasUnknownPrice ? "Hesaplanacak" : formatPrice(shipping.fee)}</b></p><p><span>Toplam</span><b>{hasUnknownPrice ? "—" : formatPrice(total)}</b></p><button disabled>Siparişi Tamamla</button><small className="checkout-disabled-note">Ödeme sağlayıcısı bağlandığında bu buton aktif olacak. Sipariş kayıt endpointi hazır.</small></aside>
  </main></>;
}
