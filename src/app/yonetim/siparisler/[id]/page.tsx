...
export default async function AdminOrderDetail({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="admin-page"><div className="admin-breadcrumb"><Link href="/yonetim/siparisler">← Siparişler</Link></div><div className="admin-page-head"><div><span>SİPARİŞ</span><h1>{order.order_no}</h1></div><p>{new Date(order.created_at).toLocaleString("tr-TR")} · {order.guest_email||"Üye kullanıcı"}</p></div>
    ...
    <section className="admin-three-grid"><article><h2>Adresler</h2>{addresses?.map(a=><p key={a.id}><b>{a.kind}</b><br/>{a.full_name}{a.company_name?<><br/>{a.company_name}</>:null}<br/>{a.address_line}<br/>{a.district} / {a.city}{a.tax_number?<><br/>VKN/TCKN: {a.tax_number}</>:null}</p>)}</article>
      <article><h2>Kargo</h2>
        <form action={createBasitKargoShipment} className="admin-editor"><input type="hidden" name="order_id" value={id}/><select name="handler_code" defaultValue="ECONOMIC"><option value="ECONOMIC">En ekonomik</option><option value="FAST">En hızlı</option><option value="ARAS">Aras</option><option value="MNG">MNG</option><option value="YURTICI">Yurtiçi</option><option value="SURAT">Sürat</option><option value="PTT">PTT</option></select><div className="form-grid"><input name="height" type="number" min="1" defaultValue="10" placeholder="Yükseklik cm"/><input name="width" type="number" min="1" defaultValue="15" placeholder="Genişlik cm"/><input name="depth" type="number" min="1" defaultValue="5" placeholder="Derinlik cm"/><input name="weight" type="number" min="0.1" step="0.1" defaultValue="1" placeholder="Ağırlık kg"/></div><button type="submit" disabled={order.payment_status!=="paid"}>BasitKargo kodu oluştur</button>{order.payment_status!=="paid"&&<small>Ödeme tamamlanınca aktif olur.</small>}</form>
        <hr/>
