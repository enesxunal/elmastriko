...
declare
  ...
  v_shipping_fee_config numeric(12,2);
  v_available integer;
...
begin
  ...
  loop
    ...
    if v_variant.id is null then
      raise exception 'variant_not_found:%', v_product.slug;
    end if;

    select greatest(stock - reserved, 0) into v_available
  ...
  select nullif(value->>'shippingFee','')::numeric
    into v_shipping_fee_config
  ...
  if v_subtotal >= 5000 then
    v_shipping_fee := 0;
  elsif v_shipping_fee_config is null then
    raise exception 'shipping_fee_not_configured';
  else
    v_shipping_fee := v_shipping_fee_config;
  end if;
